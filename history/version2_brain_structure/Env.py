...
#-----------------------------------------------------------------------------------------------------------------------
from history.version2_brain_structure.utils.EggTool import EggTool
from history.version2_brain_structure.egg.Egg import Egg
from history.version2_brain_structure.Frog import Frog
...
class Env(object):
    ...
    def __init__(self,tk:Tk,canvas:Canvas,brain_structure:BrainStructure):
        self.tk=tk#Tk()
    ...
    def run(self):#运行
        ...
        _round=1#运行次数
        while _round<=ROUND_PER_PROCESS:
            ...
            self.brain_structure.drawBrain(frog=self.frogs[0])
